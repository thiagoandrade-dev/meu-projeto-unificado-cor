...
export interface Imovel {
  _id: string;
  grupo: number; // Grupos de 12 a 18 (pares e ímpares)
  bloco: string; // A, B, C, D, E, F, G (G não permitido para grupos pares)
  andar: number; // 1-28 para grupos pares, 1-36 para grupos ímpares
  apartamento: number; // 8 aptos por andar (grupos pares), 4 aptos por andar (grupos ímpares)
  configuracaoPlanta: 'Padrão (2 dorms)' | '2 dorms + Despensa' | '2 dorms + Dependência' | 'Padrão (3 dorms)' | '3 dorms + Dependência';
  areaUtil: number; // 82, 84, 86, 125 ou 135 m² conforme configuração
  numVagasGaragem: number; // 1 a 3 vagas
  tipoVagaGaragem: 'Coberta' | 'Descoberta'; // Coberta obrigatória para mais de 1 vaga
  preco: number;

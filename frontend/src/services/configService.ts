...
class ConfigService {
  ...
  async getEmailConfig(): Promise<EmailConfig> {
    try {
      ...
      throw new Error(getErrorMessage(error) || 'Erro ao obter configurações de e-mail');
    }
  }
  ...
  async saveEmailConfig(config: EmailConfig): Promise<ApiResponse> {
    try {
      ...
      throw new Error(getErrorMessage(error) || 'Erro ao salvar configurações de e-mail');
    }
  }
  ...
  async testEmailConfig(testEmail: string): Promise<ApiResponse> {
    try {
      ...
      throw new Error(getErrorMessage(error) || 'Erro ao testar configuração de e-mail');
    }
  }
}

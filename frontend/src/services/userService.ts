...
export const userService = {
  ...
  // Solicitar reset de senha para um usuário específico (apenas admin)
  requestUserPasswordReset: async (userId: string): Promise<{ mensagem: string; usuario: { id: string; nome: string; email: string } }> => {
    try {
      const response = await apiClient.post(`/usuarios/${userId}/reset-senha`);
      return response.data;
    } catch (error) {
      throw handleApiError(error, 'solicitar reset de senha do usuário');
    }
  },
  ...
  // Atualizar usuário
  update: async (id: string, userData: Partial<User>): Promise<{ mensagem: string; usuario: User }> => {
    try {
